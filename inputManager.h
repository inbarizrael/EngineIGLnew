...
static void glfw_mouse_press(GLFWwindow* window, int button, int action, int modifier)
{
  ...
  if (action == GLFW_PRESS)
  {
	  ...
	  int newId = 0;
	  //store the previous selected object id to be selected in case we haven't found a new one (Picking returned (-1)) 
	  int i = 0, savedIndx = scn->selected_data_index;
	  // find the closeset object from the origin of the camera
	  ...
	  { 
		  ...
		  if (distance > 0 && distance < minDistance) {
			  ...
			  newId = i;
	//		  std::cout << "ID of picked closest item : " << newId << std::endl;
			  found = true;
		  }
	  ...
	  if(!found)
	  {
		  ...
		  std::cout << "insertedID :" << newId << std::endl;
		  scn->selected_data_index = newId; 
	  }
