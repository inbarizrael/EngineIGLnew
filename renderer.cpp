...
void Renderer::MouseProcessing(int button)
{
	...
	if (button == 1)
	{
		...
		scn->data().MyPreTranslate(Eigen::Vector3f(-xrel / 2000.0f, 0, 0));
		scn->data().MyPreTranslate(Eigen::Vector3f(0,yrel / 2000.0f,0));
...
double Renderer::Picking(double newx, double newy)
{
	...
	// if an object hasn't been found exit and return -1//
	if (!(
		igl::unproject_onto_mesh(Eigen::Vector2f(x, y), view,
			core().proj, core().viewport, scn->data().V, scn->data().F, fid, bc)))
	{
		return -1;
	}
	// finding vertex ids for the relevant face
		int v1 = scn->data().F.row(fid)[0];
		int v2 = scn->data().F.row(fid)[1];
		int v3 = scn->data().F.row(fid)[2];
	// storing the vertices as vectors
		Eigen::Vector3d ver1 = scn->data().V.row(v1);
		Eigen::Vector3d ver2 = scn->data().V.row(v2);
		Eigen::Vector3d ver3 = scn->data().V.row(v3);
	// calculating the coordinates of the point of impact
		double Px = bc(0) * ver1(0) + bc(1) * ver2(0) + bc(2) * ver3(0);
		double Py = bc(0) * ver1(1) + bc(1) * ver2(1) + bc(2) * ver3(1);
		double Pz = bc(0) * ver1(2) + bc(1) * ver2(2) + bc(2) * ver3(2);
	//storing p as a 4 dimensional vector in order to be able to translate by multiplying the point with theview matrix
		Eigen::Vector4f p((float)Px, (float)Py, (float)Pz, 1);
		p = view * p;
	// calculating the distance between the point p and the origin of the camera coordinates
